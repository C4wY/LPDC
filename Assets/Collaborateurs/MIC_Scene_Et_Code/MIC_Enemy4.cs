...
public class MIC_Enemy4 : MonoBehaviour
{
 ...
 //Ne marche pas pour la détection
    ...
    void Update()
    {
        ...
        {
            if (IsPlayerOnRightSide() && spriteRenderer.flipX)
            {
                FaireDégâts();
            break; 
            }
            else if (!IsPlayerOnRightSide() && !spriteRenderer.flipX)
            {
                FaireDégâts();
            break;
            }
    ...
    {
        return joueur.transform.position.x > transform.position.x;
    }
}
